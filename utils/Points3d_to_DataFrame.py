...
def points3d_arr_to_df(points_3d, bp_name_list):
    ...
    Returns:
        ...
        '''
    points_3d_reshaped = points_3d.reshape(points_3d.shape[0], 
                                           points_3d.shape[1]*points_3d.shape[2])
    coord_names = ['x', 'y', 'z']
    hierarchical_index = pd.MultiIndex.from_product([bp_name_list, coord_names],
                           names=['bodyparts', 'coords'])
    df = pd.DataFrame(data = points_3d_reshaped, 
                      columns = hierarchical_index)
    return df
